"""This module defines classes to represent an Express Pipeline."""
import logging
import json
import typing as t
...
@dataclass
class ExpressComponentOperation(ExpressComponent):
    """
    Class representing an operation for an Express Component in a Kubeflow Pipeline. An operation
    is a representation of a function that will be executed as part of a pipeline.

    Arguments:
        yaml_spec_path: The yaml file containing the express component specification.
        args: A dictionary containing the input arguments for the operation.
    ...
    Note:
        - An Express Component operation is created by defining an Express Component and its input
            arguments.
    ...
    p_volumes: t.Dict[str, k8s_client.V1Volume] = None
    ephemeral_storage_size: str = None

    def __post_init__(self):
        super().__init__(self.yaml_spec_path)


class ExpressPipeline:
    """Class representing an Express Pipeline"""
    ...
    def _validate_pipeline_definition(
        ...
        for express_component_operation in express_components_operation:
            if not load_component:
                for (
                    ...
                    if subset.fields != available_subsets[subset_name].fields:
                        raise InvalidPipelineDefinition(
                            ...
                            f" '{express_component_operation.name}' component does not match the"
                ...
                # TODO: allow loading component to have missing input subset
                available_subsets.update(express_component_operation.input_subsets)
            available_subsets.update(express_component_operation.output_subsets)
    ...
    def compile_pipeline(
        ...
        *,
        ...
        Args:
            ...
            express_components_operation: A list of ExpressComponent operations that define
        ...
        Example usage:
            To create a pipeline, first define the pipeline components as ExpressComponentOperation
             objects, then call this function to compile the pipeline function:

            express_components_operations = [
                ExpressComponentOperation(yaml_spec_path=<path_to_yaml>, args:{batch_size: 8, ...}),
                ExpressComponentOperation(...),
            ]

            pipeline_function = create_pipeline(
                name='MyPipeline',
                 description='A description of my pipeline',
                express_components_operations=express_components_operations,
        ...
        def _get_component_function(
            ...
            Load the Kubeflow component based on the specification from the express component
             operation.
            Args:
                express_component_operation (ExpressComponentOperation): The express component
                 operation.
            Returns:
        ...
        def _set_task_configuration(task, express_component_operation):
            ...
            p_volumes = express_component_operation.p_volumes
            ephemeral_storage_size = express_component_operation.ephemeral_storage_size
