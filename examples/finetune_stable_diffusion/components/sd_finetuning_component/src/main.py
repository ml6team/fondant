...
def parse_args():
    ...
    parser.add_argument('--data-manifest-gcs-path',
                        type=str,
                        required=True,
                        help='the path to the data manifest that contains information on the '
                             'training set')
    parser.add_argument('--pretrained-model-gcs-path',
                        type=str,
                        required=True,
    ...
    parser.add_argument('--random-flip',
                        type=str,
                        required=False,
                        help='whether to randomly flip the images horizontally')
    parser.add_argument('--model-gcs-path-finetuning-component',
                        type=str,
                        required=True,
...
def sd_finetuning_component(run_id: str,
                            artifact_bucket: str,
                            component_name: str,
                            project_id: str,
                            ...
                            model_gcs_path_finetuning_component: str) -> None:
    """
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        manifest_file_path = storage_helpers.download_file_from_bucket(storage_client,
                                                                       data_manifest_gcs_path,
                                                                       tmp_dir)
        ...
        finetuned_model_name = f"{model_to_finetune_name}-tmp_dir-{dataset_creation_run_id}"
        ...
        finetuned_model_gcs_uri = f"gs://{artifact_bucket}/{artifact_bucket_blob_path}/" \
        ...
        # Get index list
        index_list = parquet_helpers.get_column_list_from_parquet(index_id_path,
                                                                  column_name='index')
        filters = (pc.field("file_id").isin(index_list))

        sd_dataset_creator = SDDatasetCreator(tmp_path=tmp_dir,
                                              img_path=imgs_tmp_path)
        ...
        for namespace in datasets_parquet_dict:
            ...
            filtered_captions = parquet_helpers.filter_parquet_file(
                file_path=caption_parquet_tmp_path,
                filters=filters
            )
            sd_dataset_creator.write_dataset(dataset_scanner=filtered_dataset,
                                             captions_scanner=filtered_captions,
                                             namespace=namespace)

        # Download model to finetune (without checkpoints)
        training_utils.download_model_without_checkpoints(pretrained_model_gcs_path,
        ...
        scheduler.add_job(func=training_utils.sync_training_checkpoints,
                          trigger='interval',
        ...
        training_cmd = ['accelerate', 'launch',
                        ...
                        '--seed', str(seed),
                        '--pretrained_model_name_or_path', finetuned_model_path,
                        '--dataset_name', imgs_tmp_path,
                        ...
                        '--output_dir', model_to_finetune_path
                        ]

        # Add arguments defined with 'store_true' action if they are set to "True"
        optional_args = dict_of(random_flip, center_crop, use_ema, gradient_checkpointing, scale_lr)
        training_cmd.extend([f'--{arg}' for arg, value in optional_args.items() if value])
        ...
        logger.info('Training complete, waiting for model and checkpoint sync to upload the latest'
        ...
        subprocess.run(['gsutil', '-m', 'rsync', '-r', finetuned_model_path,  # nosec
                        finetuned_model_gcs_uri], check=True)

        # Write manifest to outputPath
        Path(model_gcs_path_finetuning_component).parent.mkdir(parents=True, exist_ok=True)
