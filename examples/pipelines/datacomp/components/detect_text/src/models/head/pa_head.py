...
class PA_Head(nn.Module):
    def __init__(
        self, in_channels, hidden_dim, num_classes, loss_text, loss_kernel, loss_emb
    ):
    ...
    def get_results(self, out, img_meta, cfg):
        ...
        if with_rec:
            outputs.update(dict(label=label, bboxes_h=bboxes_h, instances=instances))
    ...
    def loss(self, out, gt_texts, gt_kernels, training_masks, gt_instances):
        ...
        loss_emb = self.emb_loss(
            embs, gt_instances, gt_kernels[:, -1, :, :], training_masks, reduce=False
        )
