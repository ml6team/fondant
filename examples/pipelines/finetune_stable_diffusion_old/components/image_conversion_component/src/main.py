...
def parse_args():
    ...
    parser.add_argument(
        "--data-manifest-path-image-conversion-component",
        type=str,
        required=True,
...
def dataset_image_conversion_component(
    run_id: str,
    artifact_bucket: str,
    component_name: str,
    project_id: str,
    ...
    data_manifest_path_image_conversion_component: str,
) -> None:
    """
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        dataset_parquet_blob_path = os.path.join(
            artifact_bucket_blob_path, "dataset.parquet"
        )
        converted_images_blob_path = os.path.join(
            artifact_bucket_blob_path, "converted_images"
        )
        converted_images_gcs_uri = (
            f"gs://{artifact_bucket}/{converted_images_blob_path}"
        )
        logger.info(
        ...
        parquet_dataset = ds.dataset(dataset_parquet_prev_tmp_path).scanner(
            columns=["file_extension", "file_id", "file_uri", "file_size"]
        )
        ...
        image_converter.start(
            svg_image_height=svg_image_height, svg_image_width=svg_image_width
        )
        ...
        Path(data_manifest_path_image_conversion_component).parent.mkdir(
            parents=True, exist_ok=True
        )
        Path(data_manifest_path_image_conversion_component).write_text(
            data_manifest.to_json()
        )

        logger.info(
            "Manifest file written to %s", data_manifest_path_image_conversion_component
        )
...
if __name__ == "__main__":
    ...
    dataset_image_conversion_component(
        ...
        data_manifest_path_image_conversion_component=args.data_manifest_path_image_conversion_component,
    )
