...
def parse_args():
    ...
    parser.add_argument(
        "--data-manifest-path-clip-retrieval-component",
        type=str,
        required=True,
...
def clip_retrieval_component(
    ...
    logger.info(
        "Laion index download complete: it took %s minutes to download the laion indices",
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        knn_retrieval_blob_path = os.path.join(
            artifact_bucket_blob_path, "clip_knn.parquet"
        )
        centroid_retrieval_blob_path = os.path.join(
            artifact_bucket_blob_path, "clip_centroid.parquet"
        )
        ...
        logger.info(
            "A total of %s urls will be requested using the centroid approach.",
            nb_images_centroid,
        )
        logger.info(
            "A total of %s urls will be requested per individual image (total of %s images)"
        ...
        clip_retrieval_runner = ClipRetrievalLaion5B(
            laion_index_path=laion_indices_folder, laion_index_folder=laion_index_folder
        )
        ...
        for embedding_file in tqdm(os.listdir(tmp_embedding_dir_path)):
            ...
            results = clip_retrieval_runner.run_query(
                knn_service=knn_service,
        ...
        Path(data_manifest_path_clip_retrieval_component).parent.mkdir(
            parents=True, exist_ok=True
        )
        Path(parquet_path_clip_knn_retrieval).parent.mkdir(parents=True, exist_ok=True)
        Path(parquet_path_clip_centroid_retrieval).parent.mkdir(
            parents=True, exist_ok=True
        )
        # Write Parquet url files to outputPath
        Path(data_manifest_path_clip_retrieval_component).write_text(
        ...
        logger.info(
            "Manifest file written to %s", data_manifest_path_clip_retrieval_component
        )
...
if __name__ == "__main__":
    ...
    clip_retrieval_component(
        ...
        data_manifest_path_clip_retrieval_component=args.data_manifest_path_clip_retrieval_component,
