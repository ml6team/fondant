...
def parse_args():
    ...
    parser.add_argument('--min-length',
                        type=int,
                        required=True,
                        help='The minimum caption length')
    parser.add_argument('--max-length',
                        type=int,
                        required=True,
                        help='The maximum caption length')
    parser.add_argument('--beams',
                        type=int,
                        required=True,
                        help='The blip beam parameters')
    ...
    parser.add_argument('--data-manifest-path-caption-component',
                        type=str,
                        required=True,
...
def dataset_caption_component(run_id: str,
                              artifact_bucket: str,
                              component_name: str,
                              project_id: str,
                              min_length: int,
                              ...
                              data_manifest_path_caption_component: str) -> None:
    """
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for namespace, dataset_parquet_tmp_path in dataset_parquet_path_dict.items():
            logger.info('Starting caption and upload job for for dataset with namespace "%s"',
                        namespace)
            ...
            kfp_image_captioner.start(min_length=min_length,
            ...
            captions_parquet_blob_path = os.path.join(artifact_bucket_blob_path,
                                                      f'captions_{namespace}.parquet')
            ...
            logger.info('Caption and upload job for dataset with namespace "%s" is complete',
                        namespace)

            # Update manifest
            data_manifest.associated_data.caption[namespace] = \
        ...
        Path(data_manifest_path_caption_component).parent.mkdir(parents=True, exist_ok=True)
