...
def test_new_pipeline_interface():
    ...
    # Get component_ops from pipeline
    component_ops = [
        element[1]["fondant_component_op"] for element in list(pipeline._graph.items())
    ]

    assert component_ops[0].schema == {"image": "binary"}
    assert component_ops[1].consumes == {"images_data": "image"}
    assert component_ops[1].produces == {"captions": "text"}
    assert component_ops[2].consumes == {"text_data": "text"}
    assert component_ops[2].produces is None
    assert component_ops[3].schema == {"image": "binary", "caption": "string"}
